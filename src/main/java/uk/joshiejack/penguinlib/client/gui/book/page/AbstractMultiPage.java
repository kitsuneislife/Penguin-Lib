...
public abstract class AbstractMultiPage<P> extends AbstractPage {
    ...
    public AbstractMultiPage(Component name, int perPage) {
        super(name);
    ...
    public abstract static class Left<P> extends AbstractMultiPage<P> {
        public Left(Component name, int perPage) {
            super(name, perPage);
        }
        ...
        public void initLeft(Book book, int left, int top) {
            ...
            if (page != 0)
                book.addRenderableWidget(new ArrowButton.Left(book, left + 20, top + 154, (button) -> back(book)));
            //IF PAGE != MAX_PAGE, Display Right Button
            int maxPage = (Math.max(0, (int) Math.ceil(((double) entries.size())/perPage) - 1));
            if (page != maxPage)
                book.addRenderableWidget(new ArrowButton.Right(book, left + 130, top + 154, (button) -> forward(book)));
            //IF PAGE != 0, Display Left Button
    ...
    public abstract static class Right<P> extends AbstractMultiPage<P> {
        public Right(Component name, int perPage) {
            super(name, perPage);
        }
        ...
        public void initRight(Book book, int left, int top) {
            //IF PAGE != 0, Display Left Button
            if (page != 0)
                book.addRenderableWidget(new ArrowButton.Left(book, left + 10, top + 154, (button) -> back(book)));
            //IF PAGE != MAX_PAGE, Display Right Button
            entries = getEntries();
            int maxPage = (Math.max(0, (int) Math.ceil(((double) entries.size())/perPage) - 1));
            if (page != maxPage)
                book.addRenderableWidget(new ArrowButton.Right(book, left + 119, top + 154, (button) -> forward(book)));
            //IF PAGE != 0, Display Left Button
            for (int i = 0; i < perPage; i++) {
                int id = (page * perPage) + i;
                if (id < entries.size())
    ...
    public abstract static class Both<P> extends AbstractMultiPage<P> {
        public Both(Component name, int perPage) {
            super(name, perPage);
        }
